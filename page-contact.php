...
<body class="contact-body">
  <?php 
    switch($mode){
      case 'input': ?>
        <h1 class="contact-h1">お問い合わせ</h1> <?php ;
        break;
      case 'confirm': ?>
        <h2 class="contact-h2">お問い合わせ内容をご確認ください</h2> <?php ;
        break;
      case 'send': ?>
        <h2 class="contact-h2">送信完了</h2> <?php ;
        break;
      default: break;
    }
  ?>
  <div class="container">
    <div class="wrap">
      <?php if($mode == 'input'){?>
        ...
        <div class="form">
          <form action="./" method="post">
            <h3 class="input-name contact-h3">お名前<span class="mandatory">必須</span>
              <?php  
                if($err_name){
            ...
            <h3 class="contact-h3">Eメール<span class="mandatory">必須</span>
                <?php  
                  if($err_email){
            ...
            <h3 class="contact-h3">お問い合わせ内容<span class="mandatory">必須</span>
              <?php  
                if($err_message){
      ...
      <?php }else if($mode == 'confirm'){ ?>
        ...
        <div class="form">
          <form action="./" method="post">
            ...
            <h3 class="confirm-name border-bottom contact-h3">お名前</h3>
            <p><?php echo $_SESSION['name']  ?><span class="mr">様</span></p>
            <?php 
              if($_SESSION['company']){ ?>
              <h3 class="border-bottom contact-h3">会社名</h3>
              <p><?php echo $_SESSION['company']  ?><span class="mr">様</span></p>
            <?php } ?>  
            <h3 class="border-bottom contact-h3">Eメール</h3>
            <p><?php echo $_SESSION['email'] ?></p>
            <?php 
              if($_SESSION['tell']){ ?>
              <h3 class="border-bottom contact-h3">電話番号</h3>
              <p class="contact-p"><?php echo $_SESSION['tell']  ?></p>
            <?php } ?>  
            <h3 class="border-bottom contact-h3">お問い合わせ内容</h3>
            <p class="contact-p"><?php echo nl2br($_SESSION['message']) ?></p>
      ...
      <?php } else{ ?>
        ...
        <!-- 送信完了画面 -->
        <div class="form">
          <p class="contact-p"><?php echo $_SESSION['name']; $_SESSION['name']    = "";?>
            様、お問い合わせありがとうございます。</p>
          <br>
          <p class="contact-p">ご入力いただいたEメールアドレス宛に確認用メールを送信いたしました。</p>
          <p class="contact-p">お問い合わせ内容につきまして確認出来次第返信させていただきます。</p>
          <br>
          <p class="contact-p">今後ともよろしくお願い申し上げます。</p>
          <br>
          <p class="contact-p">平島 裕司郎</p>
          <form action="./" method="post">
            <input class="submit" type="submit" name="back" value="戻る">
          </form>
        </div>
